...
import { loadEnvFile, validateEnvironment } from './sync-secrets.js';
...
function syncSecrets(envVars) {
	console.log('🔄 Cloudflare secretsを同期中...');
	...
	secretKeys.forEach(key => {
		if (envVars[key]) {
			try {
				...
				execSync(`echo "${envVars[key]}" | wrangler secret put ${key}`, {
			...
			console.warn(`  ⚠️ ${key} not found in environment file`);
		}
	});
}
...
function initializeDatabase() {
	...
	try {
		// 本番データベースのテーブル存在確認
		const checkResult = execSync('wrangler d1 execute trends_list --remote --command="SELECT name FROM sqlite_master WHERE type=\'table\' AND name=\'users\';"', {
			stdio: 'pipe',
			encoding: 'utf8'
		});
		...
		if (!hasUsers) {
			...
			execSync('wrangler d1 execute trends_list --remote --file=./schema.sql', {
				stdio: 'inherit'
			});
...
function deployToCloudflare() {
	...
	try {
		// ビルドとデプロイを実行
		execSync('npm run wrangler:deploy', {
			stdio: 'inherit'
		});
...
function testDeployment() {
	...
	try {
		...
		if (nameMatch) {
			...
			execSync(`curl -s -o /dev/null -w "%{http_code}" ${deployedUrl} || echo "接続テスト完了"`, {
				stdio: 'inherit'
			});
		}
	} catch (error) {
		console.warn('⚠️ テスト中にエラーが発生しましたが、デプロイは成功している可能性があります');
	}
}
...
function main() {
	...
	if (!fs.existsSync(envFile)) {
		...
		console.log('cp .env.example .env.prod を実行して本番用設定を作成してください');
		process.exit(1);
	}
	...
	try {
		...
		console.log('- GitHub OAuth認証が正しく設定されているか確認');
